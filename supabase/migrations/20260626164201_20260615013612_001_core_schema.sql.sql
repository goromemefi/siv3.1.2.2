/*
# SI Building Solutions ERP — Core Schema (Migration 001)

## Overview
Creates the foundational tables for the entire ERP system including:
...
### User & Auth
- `profiles` — Extended user profile (name, role, avatar, tenant_id)

### Product Catalog
- `categories` — Product categories (hierarchical)
- `brands` — Product brands/manufacturers
- `products` — Master product catalog with SKU, barcode, pricing
- `product_variants` — Product variants

### Inventory
- `warehouses` — Physical warehouse locations
- `inventory_items` — Stock levels per product per warehouse
- `stock_movements` — Audit trail of all stock changes
- `damage_records` — Damaged/lost stock records

### Suppliers & Customers
- `suppliers` — Supplier master data
- `customers` — Customer master (contractors, builders, etc.)
- `customer_notes` — CRM notes

## Security
- RLS enabled on all tables
- Authenticated users can read/write all data (internal ERP)

## Notes
- tenant_id included for multi-tenant SaaS readiness
- Monetary values stored in decimal(15,2)
*/
